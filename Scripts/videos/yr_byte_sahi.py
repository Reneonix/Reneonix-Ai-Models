"""
Two-model pipeline: YOLO (SAHI-tiled, 6 tiles) for detection/localization only
+ ByteTrack for persistent per-object tracking + ResNet-50 for the final
classification of each tracked object's cropped region.

A middle ground between the other two variants in this folder:

  - yr_sahi_botsort.py: 6-tile SAHI + BoT-SORT+ReID (best
    small-object recall + most ID-stable in dense scenes, slowest, ~9 FPS)
  - yr_byte.py: no SAHI + ByteTrack (fastest, ~23 FPS, worse
    on small/distant objects)
  - THIS SCRIPT: 6-tile SAHI + ByteTrack - same tile grid/small-object
    recall as the BoT-SORT variant, but keeps ByteTrack's cheap motion/IoU-only
    tracking instead of BoT-SORT+ReID's appearance-embedding overhead
    (no ONNX ReID network to run every frame). The 6 tiles are still run
    as a single batched YOLO forward pass (batch size 6).

WHY YOLO+RESNET AT ALL (unchanged from the other variants):
...
augmentation (src/exp004_resnet50.py, 99.4% val accuracy), while YOLO+SAHI is
used purely to localize (its own predicted class is discarded for the final
label - only used internally to help SAHI's tile-overlap NMS merge).
...
CONF = 0.6        # minimum YOLO detection confidence to keep a box (localization only)
...
TILE_COLS = 3          # 3x2 = 6 tiles per frame - same grid as the BoT-SORT SAHI variant
TILE_ROWS = 2
TILE_OVERLAP = 0.4  # 35% overlap between neighboring tiles (raised from 0.2) - widens the
                         # shared strip between tiles so larger boundary-straddling objects still
                         # land whole inside it instead of getting clipped at a tile edge
NMS_IOU = 0.5
SAHI_BATCH_SIZE = 6     # all 6 tiles inferred in a single explicit batch (== tile count)
...
def parse_args():
    parser = argparse.ArgumentParser(description="YOLO(+6-tile SAHI) detect -> ByteTrack -> ResNet classify pipeline")
...
def sahi_infer(model, frame, tile_boxes):
    ...
    t0 = time.time()
    results = model.predict(tiles, imgsz=IMGSZ, conf=CONF, device=DEVICE, batch=SAHI_BATCH_SIZE, verbose=False)
...
def update_tracker(tracker, boxes, scores, cls, frame):
    """Feeds our own SAHI-merged detections into Ultralytics' real BYTETracker (the same class
...
def draw_tile_grid(frame, tile_boxes):
    """Faint overlay showing the current tile-slicing grid, for visual sanity-checking."""
...
def main():
    ...
    # visible playback. No tracker.reset() afterward: this frame's real detections seed real
    # tracks, which is harmless (those objects get counted normally) - the one tradeoff is this
    ...
    window = f"YOLO(6-tile SAHI)+ByteTrack+ResNet - {os.path.basename(video_path)}"
    ...
    print("\n" + "=" * 70)
    print("YOLO (6-TILE SAHI) + BYTETRACK + RESNET PIPELINE - BENCHMARK & COUNT REPORT")
    print("=" * 70)

"""
Live CAMERA inference + full benchmark report using SAHI (Slicing Aided Hyper
Inference) - https://roboflow.com/how-to-use-sahi/yolov8 /
https://learnopencv.com/slicing-aided-hyper-inference/

Same as video_predict_sahi.py, with the one change requested: the source is
your laptop's webcam instead of a video file. Each frame is:
...
TARGET_FPS = 30.0        # fallback pacing FPS - webcams frequently report an
                         # unreliable (or 0) CAP_PROP_FPS, unlike video files,
                         # so this is used whenever the camera's own value looks bad
...
def load_validation_accuracy():
    ...
    single-pass validation - not a live measurement of SAHI-tiled accuracy on the
    camera feed (no ground truth exists for a live feed to measure that directly)."""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Live YOLO + SAHI camera inference and benchmark report")
...
def main():
    ...
    # DESIGN RULE (unchanged from video_predict_sahi.py): always read the source's own
    ...
    print(f"Camera FPS: {src_fps:.2f}  |  Input resolution: {width}x{height}")
    ...
    window = f"Waste Detection (SAHI) - Live Camera {camera_index}"
    cv2.namedWindow(window, cv2.WINDOW_NORMAL)

    # per-frame timing buckets (all in ms)
    inference_times = []    # the single batched model.predict() call over all 6 tiles
    e2e_times = []            # capture -> batched inference -> merge/NMS -> draw -> display
    ...
    print("\n" + "=" * 60)
    print("SAHI LIVE CAMERA BENCHMARK REPORT")
    print("=" * 60)
    ...
    print(f"not a direct measurement of SAHI-tiled accuracy on this live camera session:")
    ...
    print()
    summarize("End-to-end latency per frame (capture + batched inference + merge/NMS + draw + display)", e2e_times)
    ...
    print(f"Live display was paced toward {src_fps:.2f} FPS; "
          f"{'model kept up in real time' if achievable_fps >= src_fps else 'SAHI pipeline is SLOWER than target FPS - live display lagged behind real time'}.")
    print("=" * 60)
